...
app.post('/api/users/me/photo', authRequired, upload.single('photo'), async (req, res, next) => {
  try {
    ...
    const safeName = (req.file.originalname || 'photo').replace(/[^a-zA-Z0-9._-]/g, '') || `photo${path.extname(req.file.originalname || '.jpg')}`
